...
def visualize_lip_detection(video_path, detector, n_frames=6):
    """
    Ambil beberapa frame representatif dari video, deteksi bibir,
    gambar bounding box + landmark, dan kembalikan:
      - annotated_frames : list of BGR image dengan bbox
      - lip_crops        : list of BGR 96×96 lip crop
      - detect_ok        : True jika minimal 1 frame berhasil
    """
    frames = extract_all_frames(video_path)
    if not frames:
        return [], [], False

    # Pilih n_frames frame tersebar merata
    indices = np.linspace(0, len(frames) - 1, n_frames, dtype=int)

    annotated, crops, detect_ok = [], [], False

    for fi in indices:
        frame = frames[fi].copy()           # sudah 224×224 dari extract_all_frames
        h, w  = frame.shape[:2]
        ...
        if result.face_landmarks:
            ...
            # ── Gambar semua landmark wajah (titik kecil abu-abu) ──
            for idx in range(len(lm)):
                px, py = int(lm[idx].x * w), int(lm[idx].y * h)
                cv2.circle(frame, (px, py), 1, (160, 160, 160), -1)

            # ── Gambar landmark bibir (titik hijau terang) ──
            for idx in LIP_LANDMARKS:
                px, py = int(lm[idx].x * w), int(lm[idx].y * h)
                cv2.circle(frame, (px, py), 2, (0, 255, 80), -1)

            # ── Bounding box bibir (kotak merah) ──
            cv2.rectangle(frame, (x0, y0), (x1, y1), (0, 60, 255), 2)

            # ── Label di atas kotak ──
            label     = 'LIP ROI'
            (tw, th), _ = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.45, 1)
            cv2.rectangle(frame, (x0, y0 - th - 6), (x0 + tw + 4, y0), (0, 60, 255), -1)
            cv2.putText(frame, label, (x0 + 2, y0 - 4),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255, 255, 255), 1)

            # ── Ukuran crop ──
            crop_w, crop_h = x1 - x0, y1 - y0
            size_txt = f'{crop_w}x{crop_h}'
            cv2.putText(frame, size_txt, (x0, y1 + 14),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.40, (0, 200, 255), 1)

            # ── Frame index ──
            cv2.putText(frame, f'f={fi}', (4, h - 6),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.38, (200, 200, 200), 1)

            # Crop bibir → resize 96×96
            crop_raw = frames[fi][y0:y1, x0:x1]
            lip_crop = cv2.resize(crop_raw, LIP_SIZE) if crop_raw.size > 0 else \
                       np.zeros((*LIP_SIZE, 3), dtype=np.uint8)
            crops.append(cv2.cvtColor(lip_crop, cv2.COLOR_BGR2RGB))
        else:
            # Frame tanpa deteksi — overlay teks
            cv2.putText(frame, 'No face detected', (5, 20),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
            crops.append(np.zeros((*LIP_SIZE, 3), dtype=np.uint8))

        annotated.append(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))

    return annotated, crops, detect_ok
...
if uploaded:
    ...
    # ── Lip Detection Preview ────────────────────────────────────────
    st.subheader('🔬 Verifikasi Deteksi Bibir')
    st.caption('Pastikan kotak merah menempel di area bibir — ini yang digunakan model.')

    with st.spinner('Mendeteksi bibir dari frame sampel...'):
        ann_frames, lip_crops, det_ok = visualize_lip_detection(tmp_path, detector, n_frames=6)

    if not det_ok:
        st.error('❌ Bibir tidak terdeteksi di frame manapun. '
                 'Pastikan wajah terlihat jelas dan menghadap kamera.')
    else:
        # Baris 1: frame annotated dengan bounding box
        st.markdown('**Frame asli + bounding box bibir** (merah = ROI · hijau = landmark bibir)')
        cols_ann = st.columns(len(ann_frames))
        for col, img in zip(cols_ann, ann_frames):
            col.image(img, use_container_width=True)

        # Baris 2: hasil crop 96×96
        st.markdown('**Hasil crop 96×96** — input ke model')
        cols_crop = st.columns(len(lip_crops))
        for col, crop in zip(cols_crop, lip_crops):
            col.image(crop, use_container_width=True)

        st.success(f'✅ Bibir terdeteksi di {sum(c.any() for c in lip_crops)}/{len(lip_crops)} '
                   f'frame sampel.')
